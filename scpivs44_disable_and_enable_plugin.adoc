...
. 형식을 사용하여 SnapCenter VMware 플러그인 관리 GUI에 `\https://<OVA-IP-address>:8080`로그인합니다. 배포 시 설정된 admin 사용자 이름과 암호 및 유지보수 콘솔을 사용하여 생성된 MFA 토큰으로 로그인합니다.
+
...
... VM의 전원을 끕니다.
... 왼쪽 탐색 화면에서 SnapCenter VMware 플러그인 인스턴스(가상 어플라이언스를 구축할 때 사용된 파일 이름)를 마우스 오른쪽 버튼으로 `.ova` 클릭하고 * Delete from Disk * 를 선택합니다.
